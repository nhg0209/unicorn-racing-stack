...
def load_params():
    ...
    # ...except the three acceleration limits, which the NODE no longer takes from this yaml:
    # since the race-day re-grip change they come from config/<SIM|CAR>/veh_dyn_info so that one
    # k in ggv.csv reaches the avoidance planner too, and the yaml values are fallbacks for an
    # unreadable config. This harness builds the node with __new__ and assigns from `p`, so
    # without this it would go on measuring the fallbacks -- exactly the hand-copied-literal
    # drift the docstring above is about, one indirection further out.
    # CAR, and cur_vs = 3.0, to match Harness.cur_vs. Only ax_max_machines is speed-dependent
    # and the 7.0 friction bound is under it below ~11 m/s, so the speed does not bite today.
    try:
        san = load_node_module()
        p = dict(p)
        p.update(san.resolve_veh_dyn_limits(
            san.load_veh_dyn_tables(str(REPO / "stack_master/config/CAR")), 3.0))
    except Exception as e:
        print(f"  [warn] veh_dyn_info unreadable ({e}); measuring the yaml fallbacks, which is "
              f"NOT what the node runs")
    return p
