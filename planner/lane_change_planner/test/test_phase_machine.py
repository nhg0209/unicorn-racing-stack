...
class Fake:
    ...
    def __init__(self):
        ...
        # BAIL GUARD (_to_idle -> _beside_target). Abandoning the lane while the two cars overlap
        # is how a failed overtake ends in contact, so _to_idle holds the lane instead. The
        # harness had none of this state, so every path through _to_idle raised AttributeError and
        # pytest aborted COLLECTION of this file -- eleven tests ran, the rest of the phase
        # machine had no gate at all.
        self.bail_block_m = 1.5
        self.bail_hold_max_s = 3.0
...
def test_the_phase_machine():
    """Every check below, as ONE pytest test.

    This file used to run its checks at module level and end in sys.exit(). Under pytest
    that is fatal twice over: an exception in the body aborts COLLECTION of the file (which
    is how a missing harness attribute silently disabled the whole phase machine's only
    gate), and the sys.exit at the end kills the pytest process itself with an
    INTERNALERROR. The body is unchanged; it just lives in a function now.
    """
    FAILS.clear()
    ...
    assert not FAILS, f"{len(FAILS)} failing checks: {FAILS}"


if __name__ == "__main__":
    try:
        test_the_phase_machine()
