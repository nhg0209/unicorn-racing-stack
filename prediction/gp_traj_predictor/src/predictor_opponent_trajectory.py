...
class GaussianProcessOppTraj(PredictionNode):
    def __init__(self):
        super().__init__('predictor_opponent_trajectory')
        ...
        #Subscribers
        self.create_subscription(ProjOppTraj, '/proj_opponent_trajectory', self.proj_opp_traj_cb, 10)
        self.create_subscription(WpntArray, '/global_waypoints', self.glb_wpnts_cb, 10)

        # Optional rosbag input: /opp/car_state/odom_frenet
    ...
    def get_gp_opp_traj(self):
        ...
        self.opponent_trajectory = self.wait_for_message('/opponent_trajectory', OpponentTrajectory)
        proj_opp_traj = ProjOppTraj()

        while rclpy.ok():
            proj_opp_traj = self.wait_for_message('/proj_opponent_trajectory', ProjOppTraj)
    ...
    def make_opponent_trajectory_msg(self, oppwpnts_list: list, lap_count: int, raw_oppenent_traj_msg: ObstacleArray):
        ...
        opponent_trajectory_msg.opp_is_on_trajectory = raw_oppenent_traj_msg.opp_is_on_trajectory
