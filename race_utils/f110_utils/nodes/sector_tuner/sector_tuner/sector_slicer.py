...
class SectorSlicer(Node):
    ...
    def timer_callback(self):
        ...
        if self.use_gui:
            ...
            self.get_logger().warn('use_gui:=false -> skipping the slicing GUI; '
                                   'writing ONE sector over the whole track.')
    ...
    def _rebuild_for_next_use(self):
        ...
        Mirrors the ros1 sector_slicing (subprocess.Popen(finish_sector.sh)):
        the slicer wrote speed_scaling.yaml into src, then kicked off a colcon
        build so the file is available the next time the stack launches. Without
        this, a freshly-created map has no install symlink and sector_tuner /
        ot_interpolator die with KeyError: 'n_sectors'.
        """
        script = os.path.join(get_package_prefix('sector_tuner'),
                              'lib', 'sector_tuner', 'finish_sector.sh')
