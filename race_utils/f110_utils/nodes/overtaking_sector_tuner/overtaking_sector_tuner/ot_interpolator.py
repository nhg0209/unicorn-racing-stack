...
from overtaking_sector_tuner.parameter_event_handler import ParameterEventHandler
...
class OvertakingInterpolator(Node):
    ...
    Publishes:
        /global_waypoints/overtaking          : interpolated overtaking WpntArray
        /global_waypoints/overtaking/markers  : MarkerArray of the overtaking line
        /ot_sector_markers                    : MarkerArray of the sector boundaries

    Sector configuration (n_sectors, yeet_factor, spline_len, ot_sector_begin and
    the per-sector Overtaking_sectorX/ot_flag) comes from ot_sectors.yaml, loaded
    as launch parameter overrides. Setting the `save_params` parameter True writes
    the configuration back to the map's ot_sectors.yaml (unicorn-specific).
    """
    ...
    def __init__(self):
        ...
        # unicorn-specific: path to the map yaml for save-back
        try:
    ...
    def glb_wpnts_cb(self, data: WpntArray):
        """Saves the global waypoints (scaled minimum curvature)."""
    ...
    def save_yaml(self):
        """unicorn-specific: dump the current OT sector configuration to the map yaml."""
        ...
        try:
            yaml_data = {
                ...
                'ot_sector_begin': float(self.sectors_params.get('ot_sector_begin', 0.5)),
            }
            for i in range(self.n_sectors):
                sec = self.sectors_params[f"Overtaking_sector{i}"]
                yaml_data[f"Overtaking_sector{i}"] = {
                    'start': int(sec['start']),
                    'end': int(sec['end']),
    ...
    def next_sector_overtaking(self, idx):
        if idx == (self.n_sectors-1):
            return self.sectors_params["Overtaking_sector0"]['ot_flag']
        else:
            return self.sectors_params[f"Overtaking_sector{idx+1}"]['ot_flag']
    ...
    def interpolate_points(self):
        """Scales / interpolates the global waypoints."""
