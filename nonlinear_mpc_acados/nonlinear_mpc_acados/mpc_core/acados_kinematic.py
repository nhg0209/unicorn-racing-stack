        ...
        if _rc is not None:
            ...
            if r_up is not None and r_dn is not None:
                ...
                if ok_up and ok_dn:
                    ...
                    _side = +1
                    # 0822 가까운쪽 우선: w_car_safe(이중계상, ~1.25)에 탈락한
                    # 라인쪽이 물리최소(narrow_room 0.55)를 넘고, 통과쪽 우회
                    # 목표가 0.8 m 이상 더 멀면 라인쪽 서행 통과로 뒤집는다
                    # (bag 20_19: room_up 0.70 탈락 -> e_eff -1.72 헛우회 x1390).
                    try:
                        _nr0 = float(getattr(self, 'narrow_room_live', 0.55))
                        _ns = -_side
                        _nroom = ((r_up - float(extra_up)) if _ns > 0
                                  else (r_dn - float(extra_dn)))
                        _far = abs(float(e_c_obs) + _side * w_car_safe)
                        _near = abs(float(e_c_obs) + _ns * _nr0)
                        if (_nroom >= _nr0 and _far - _near > 0.8
                                and _ns not in _stuck):
                            self._narrow_pass_until = (
                                time.monotonic()
                                + float(getattr(self, 'narrow_pass_hold', 1.5)))
                            self._log.info_throttle(
                                1.0, "[MPC] 가까운쪽 우선 side=%+d (near|e|=%.2f "
                                "room=%.2f vs far|e|=%.2f) — 서행",
                                _ns, _near, _nroom, _far)
                            _side = _ns
                    except Exception:
                        pass
                elif ok_dn:
