...
- Params: there is no single convention in this subsystem, so check the node before assuming one.
  ...
  - closed_reopt (reopt_method:=closed_qp) — a FIFTH convention: its six parameters are dataclass defaults in planner/gb_optimizer/gb_optimizer/closed_reopt.py (ReoptParams), reachable from no yaml and no launch arg. Only the solver CHOICE is a launch arg. check_avoidance_margins.py parses the dataclass and reports that chain separately; today it FAILS on headroom (0.310 delivered vs 0.280 needed + 0.05 required) and is non-fatal only because local_window is still the default.
...
- ifac's own raceline peaks at |kappa| 1.448 against a curvlim of 1.5 (96.5%). NEVER gate an avoidance on an absolute curvature: on this map every line is near the limit before it starts. Compare against the CLEAN line measured the same way, and never against another solver's published output (the hump's uniform resample reads 1.4378 where the raceline it was built from measures 1.4478).
