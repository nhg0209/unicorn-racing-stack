...
class LookupSteerAngle:
    ...
    def lookup_steer_angle(self, accel, vel):
        ...
        # if (vel > lu_vs[-1]):
        #   self.logger(5, "Velocity exceeds lookup table, generating steering "
        #               "angle for v :" + str(lu_vs[-1]))

        # find closest velocities to vel
        c_v, c_v_idx = find_nearest(lu_vs, vel)

        # find two closest accelerations to accel
        c_a, c_a_idx, s_a, s_a_idx = find_closest_neighbors(
            self.lu[1:, c_v_idx + 1], accel)
        ...
        if c_a_idx == 0:
            ...
            steer_angle = np.interp(accel, [c_a, s_a],
                                    [lu_steers[c_a_idx], lu_steers[s_a_idx]])
